...
一度は4レンジ（〜13/14〜26/27〜50/51話〜）で最低本数を保証する形にしたが、
「27〜50話」は上位3,000件中の候補が109件しかなく、50本を確保するにはsplit_score
0.836→0.679まで妥協が必要で、それでもテストユーザーのデータ不足率は100%→90%
までしか改善しなかった。母集団完走率を見ても27〜50話(93.5%)・51〜100話(92.4%)の差は
1pt前後で分割の実質的な根拠が薄かったため、`api/services/profile.py`の
`EPISODE_BUCKET_DEFS`を3バケット（〜13話/14〜26話/27話〜）に統合したのに合わせて、
本スクリプトも3レンジに変更した（詳細は reports/question_pool_episode_rebalance.md）。

## 選定ロジック
1. 登録者数（anime.csv Members列）上位3,000件から選定母集団を作る
   （Type: Movie/Music/Special/Unknown除外、年代・話数・ジャンル・タイトル欠損は除外）
2. 登録者数上位200位以内から上位50本を無条件確保（認知度の担保）
3. 話数3レンジそれぞれで最低本数を保証する（reserved分もレンジ内カウントに含む）:
   〜13話 70本 / 14〜26話 70本 / 27話〜 70本
   （希少なレンジ＝候補数が少ないレンジから先に埋め、ジャンル・年代の分散上限
   （genre_cap/year_cap）を候補数の多いレンジに食い潰されないようにする。
   上限内で埋まりきらない場合は上限を外した補充パスで必ず本数を満たす）
4. 残り40本は話数レンジを問わず split_score 降順で自由選定（従来の多様化ロジックを流用）
...
EPISODE_BUCKETS: list[tuple[float, float, str, int]] = [
    # (下限, 上限, ラベル, 最低本数)。api/services/profile.py の EPISODE_BUCKET_DEFS と揃える。
    (1, 13, "〜13話", 70),
    (14, 26, "14〜26話", 70),
    (27, float("inf"), "27話〜", 70),
]

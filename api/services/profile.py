...
# 表示・特徴量ともに1本のデータで壊れるため、複数バケットの分布に置き換える）。
# 2026-08: 4バケット（〜13/14〜26/27〜50/51〜）から3バケットに変更した
# （reports/question_pool_episode_rebalance.md 参照）。27〜50話と51話〜の
# 母集団完走率の差は約1ptしかなく分割の実質的な根拠が薄い一方、質問プール20本中の
# 分母をこれ以上細分化すると「27〜50話」のデータ不足（テストユーザーの100%）が
# 解消できないため、27話〜を1つのバケットに統合した。
